...
const Services = () => {
  ...
  const emergingTech = [
    { name: "Inteligencia Artificial (IA)", icon: <Brain className="w-6 h-6 text-purple-600" /> },
    { name: "Internet de las Cosas (IoT)", icon: <Link className="w-6 h-6 text-purple-600" /> },
    { name: "Blockchain", icon: <Blocks className="w-6 h-6 text-purple-600" /> },
    { name: "Computación en la Nube", icon: <Cloud className="w-6 h-6 text-purple-600" /> },
    { name: "Firma Electrónica", icon: <FileText className="w-6 h-6 text-purple-600" /> }
  ];

  return (
    <section id="servicios" className="py-20 bg-white font-corporate">
              ...
              {globalSolutions.map((service, index) => (
                <Card key={index} className="hover:shadow-lg transition-all duration-300 group animate-fade-in">
                  <CardHeader>
                    ...
                    <CardTitle className="text-xl text-gray-900 font-corporate">{service.title}</CardTitle>
                  </CardHeader>
                  <CardContent>
